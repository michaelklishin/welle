...
(let [conn (wc/connect)]
  ...
  (deftest test-basic-store-with-all-defaults
    (let [bucket-name "clojurewerkz.welle.kv/store-with-all-defaults"
          ...
          v           "value"
          stored      (kv/store conn bucket-name k v)
          {:keys [result] :as m} (kv/fetch conn bucket-name k {:r 1})
  ...
  (deftest test-basic-store-with-text-utf8-content-type
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          v           "value"
          stored      (kv/store conn bucket-name k v {:content-type Constants/CTYPE_TEXT_UTF8})
          {:keys [result] :as m} (kv/fetch conn bucket-name k)
          ;; ex.: {:metadata {},
          ;;       :deleted? false,
  ...
  (deftest test-basic-store-with-json-content-type
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          stored      (kv/store conn bucket-name k v {:content-type Constants/CTYPE_JSON})
          {:keys [result] :as m} (kv/fetch conn bucket-name k)
          fetched     (first result)]
  ...
  (deftest test-basic-store-with-json-utf8-content-type
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          v           {:name "Riak" :kind "Data store" :influenced-by #{"Dynamo"}}
          stored      (kv/store conn bucket-name k v {:content-type Constants/CTYPE_JSON_UTF8})
          {:keys [result] :as m} (kv/fetch conn bucket-name k)
          fetched     (first result)]
  ...
  (deftest test-basic-store-with-jackson-smile-content-type
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          v           {:name "Riak" :kind "Data store" :influenced-by #{"Dynamo"}}
          stored      (kv/store conn bucket-name k v {:content-type ct})
          {:keys [result] :as m} (kv/fetch conn bucket-name k)
          fetched     (first result)]
  ...
  (deftest test-basic-store-with-application-clojure-content-type
    (let [bucket-name "clojurewerkz.welle.kv2"
          ...
          stored      (kv/store conn bucket-name k v {:content-type ct})
          {:keys [result] :as m} (kv/fetch-one conn bucket-name k)]
      ;; cannot use constant value here see https://github.com/basho/riak-java-client/issues/125
  ...
  (deftest test-basic-store-with-text-utf8-content-type-and-return-body
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          k           "store-as-utf8-text"
          v           "value"
          {:keys [result]}      (kv/store conn bucket-name k v {:content-type Constants/CTYPE_TEXT_UTF8 :return-body true})]
  ...
  (deftest test-basic-store-with-metadata
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          _           (kv/store conn bucket-name k v {:content-type Constants/CTYPE_TEXT_UTF8 :metadata metadata})
          {:keys [result] :as m} (kv/fetch conn bucket-name k)
          fetched     (first result)]
      (is (= Constants/CTYPE_TEXT_UTF8 (:content-type fetched)))
  ...
  (deftest test-basic-store-with-links
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          _           (kv/store conn bucket-name k v {:content-type Constants/CTYPE_TEXT_UTF8 :links links})
          {:keys [result] :as m} (kv/fetch-one conn bucket-name k)]
  ...
  (deftest test-fetching-a-non-existent-object
    (let [bucket-name "clojurewerkz.welle.kv"
          bucket      (wb/update conn bucket-name)
          {:keys [result]}           (kv/fetch conn bucket-name (str (UUID/randomUUID)))]
      (is (empty? result))))
  ...
  (deftest test-optimistic-fetching-of-a-single-object
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          k           "optimistic-fetch"
          v           "value"
          _                (kv/store conn bucket-name k v)
          {:keys [result]} (kv/fetch-one conn bucket-name k {:r 1})]
  ...
  (deftest test-fetch-one-with-skip-deserialize
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          v           {:name "Riak"}
          _                (kv/store conn bucket-name k v {:content-type ct})
          {:keys [result]} (kv/fetch-one conn bucket-name k {:r 1 :skip-deserialize true})]
      (is (= ct (:content-type result)))
      (is (= {} (:metadata result)))
  ...
  (deftest test-fetch-with-skip-deserialize
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          v           {:name "Riak"}
          _                (kv/store conn bucket-name k v {:content-type ct})
          {:keys [result]} (kv/fetch conn bucket-name k {:r 1 :skip-deserialize true})
          fetched          (first result)]
      (is (= ct (:content-type fetched)))
      (is (= {} (:metadata fetched)))
  ...
  (deftest test-fetching-deleted-value-with-rw=2
    (let [bucket-name "clojurewerkz.welle.kv3"
      ...
      (drain conn bucket-name)
      (Thread/sleep 150)
      (is (not (:has-value? (kv/fetch conn bucket-name k {:r 2}))))
      (kv/store conn bucket-name k v)
      (is (:has-value? (kv/fetch conn bucket-name k)))
      (kv/delete conn bucket-name k {:rw 2})
      (kv/fetch conn bucket-name k {:r 2})
  ...
  (deftest test-fetching-deleted-value-with-bucket-settings
    (let [bucket-name "clojurewerkz.welle.kv4"
      ...
      (drain conn bucket-name)
      (Thread/sleep 150)
      (is (not (:has-value? (kv/fetch-one conn bucket-name k {:r 2}))))
      (kv/store conn bucket-name k v)
      (is (:has-value? (kv/fetch-one conn bucket-name k)))
      (kv/delete conn bucket-name k {:rw 2})
      (is (not (:has-value? (kv/fetch-one conn bucket-name k {:r 2}))))))
  ...
  (deftest test-fetching-multiple-deleted-values-with-bucket-settings
    (let [bucket-name "clojurewerkz.welle.kv5"
      ...
      (doseq [[k v] key-values]
        (is (not (:has-value? (kv/fetch-one conn bucket-name k))))
        (kv/store conn bucket-name k v)
        (is (:has-value? (kv/fetch-one conn bucket-name k))))
      (kv/delete-all conn bucket-name (map first key-values))
      (doseq [[k v] key-values]
        (is (not (:has-value? (kv/fetch-one conn bucket-name k)))))))
  ...
  (deftest test-modify-with-json-content-type-and-no-existing-value
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          updated     (kv/modify conn bucket-name k f {:r 1 :w 1 :content-type "application/json"})
          {:keys [result]} (kv/fetch-one conn bucket-name k)]
  ...
  (deftest test-modify-with-json-content-type-and-one-existing-value
    (let [bucket-name "clojurewerkz.welle.kv"
          ...
          f           (fn [[m]]
                        (update-in m [:value :influenced-by] set/union #{"Java" "Haskell"}))
          _           (kv/modify conn bucket-name k f {:r 1 :w 1})
          {:keys [result]} (kv/fetch-one conn bucket-name k)]
  ...
  (deftest test-modify-vclocks
    (let [bucket-name "clojurewerkz.welle.kv.siblings"
          ...
          k           (str (UUID/randomUUID))
          append!     (fn [x] (kv/modify conn bucket-name k
                                         (fn [[o]]
          ...
          {:keys [result]} (kv/fetch conn bucket-name k {:pr 3})]
